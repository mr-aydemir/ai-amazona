...
export default function AdminDistanceSalesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const res = await fetch('/api/admin/distance-sales', { cache: 'no-store' })
        ...
        console.error('Distance sales verisi alınamadı', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/distance-sales', {
        method: 'PUT',
      ...
      toast({ title: locale === 'tr' ? 'Hata' : 'Error', description: err?.message || (locale === 'tr' ? 'Güncelleme başarısız' : 'Update failed'), variant: 'destructive' })
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>{locale === 'tr' ? 'Mesafeli Satış Sözleşmesi' : 'Distance Sales Agreement'}</CardTitle>
        </CardHeader>
        <CardContent>
