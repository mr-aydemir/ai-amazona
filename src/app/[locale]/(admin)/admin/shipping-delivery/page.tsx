...
export default function AdminShippingDeliveryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const res = await fetch('/api/admin/shipping-delivery', { cache: 'no-store' })
        ...
        console.error('Shipping & Delivery verisi alınamadı', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/shipping-delivery', {
        method: 'PUT',
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>{locale === 'tr' ? 'Kargo ve Teslimat' : 'Shipping & Delivery'}</CardTitle>
        </CardHeader>
        <CardContent>
