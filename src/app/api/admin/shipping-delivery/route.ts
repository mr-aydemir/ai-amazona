...
export async function GET() {
  try {
    ...
    console.error('[ADMIN_SHIPPING_DELIVERY_GET] Error:', error)
    return NextResponse.json({ error: 'Failed to fetch shipping & delivery page' }, { status: 500 })
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error('[ADMIN_SHIPPING_DELIVERY_PUT] Error:', error)
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: error.errors }, { status: 422 })
    }
    return NextResponse.json({ error: 'Failed to update shipping & delivery page' }, { status: 500 })
  }
}